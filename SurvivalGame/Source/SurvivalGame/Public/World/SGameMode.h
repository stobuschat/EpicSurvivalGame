...
protected:
    ...
    /* The enemy pawn class */
    UPROPERTY(EditDefaultsOnly, Category = "AI")
    TSubclassOf<class APawn> AltBotPawnClass;
...
public:
    /* Primary sun of the level. Assigned in Blueprint during BeginPlay (BlueprintReadWrite is required as tag instead of
     * EditDefaultsOnly) */
...
protected:
    ...
    /** From UT Source: Used to modify, remove, and replace Actors. Return false to destroy the passed in Actor. Default
     * implementation queries mutators. note that certain critical Actors such as PlayerControllers can't be destroyed, but we'll
     * still call this code path to allow mutators to change properties on them
     */
