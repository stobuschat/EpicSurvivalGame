...
ASGameMode::ASGameMode()
{
    ...
    AltBotPawnClass = ASurvG_ZombieCharacter::StaticClass();
...
void ASGameMode::PreInitializeComponents()
{
    ...
    GetWorldTimerManager().SetTimer(
        TimerHandle_DefaultTimer, this, &ASGameMode::DefaultTimer, GetWorldSettings()->GetEffectiveTimeDilation(), true);
}
...
void ASGameMode::StartMatch()
{
    if (!HasMatchStarted())
    {
        /* Spawn a new bot every 5 seconds (bothandler will opt-out based on his own rules for example to only spawn during night
         * time) */
...
FString ASGameMode::InitNewPlayer(
    class APlayerController* NewPlayerController, const FUniqueNetIdRepl& UniqueId, const FString& Options, const FString& Portal)
{
...
float ASGameMode::ModifyDamage(float Damage, AActor* DamagedActor, struct FDamageEvent const& DamageEvent,
        ...
        for (TActorIterator<APawn> It(GetWorld()); It; ++It)
        {
            ...
            if (OtherPawn)
            {
                const float CombinedHeight = (SpawnPoint->GetCapsuleComponent()->GetScaledCapsuleHalfHeight() +
                                                 OtherPawn->GetCapsuleComponent()->GetScaledCapsuleHalfHeight()) *
                                             2.0f;
                const float CombinedWidth = SpawnPoint->GetCapsuleComponent()->GetScaledCapsuleRadius() +
                                            OtherPawn->GetCapsuleComponent()->GetScaledCapsuleRadius();
                const FVector OtherLocation = OtherPawn->GetActorLocation();

                // Check if player overlaps the playerstart
                if (FMath::Abs(SpawnLocation.Z - OtherLocation.Z) < CombinedHeight &&
                    (SpawnLocation - OtherLocation).Size2D() < CombinedWidth)
                {
                    return false;
                }
            }
...
void ASGameMode::SpawnNewBot()
{
    ...
    {
        GetWorld()->SpawnActor<APawn>(AltBotPawnClass, SpawnTransform);
    }
}
...
UClass* ASGameMode::GetDefaultPawnClassForController_Implementation(AController* InController)
{
    ...
    if (Cast<ASurvG_ZombieAIController>(InController))
    {
        return AltBotPawnClass;
    }
...
void ASGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
    ...
    for (TActorIterator<AActor> It(GetWorld(), AActor::StaticClass()); It; ++It)
    {
        ...
        {
            // a few type checks being AFTER the CheckRelevance() call is intentional; want mutators to be able to modify, but not
            // outright destroy
